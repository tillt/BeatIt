...
constexpr double kMaxStartEndDeltaMs = 130.0;
constexpr double kMaxStartEndDeltaBeats = 0.28;
...
int main() {
...
#endif
    const std::filesystem::path audio_path = test_root / "training" / "intelligence.wav";
    ...
    if (!decode_audio_mono(audio_path.string(), &mono, &sample_rate, &decode_error)) {
        std::cerr << "Intelligence alignment test failed: decode error: " << decode_error << "\n";
        return 1;
    }
    ...
    if (!stream.request_analysis_window(&start_s, &duration_s)) {
        std::cerr << "Intelligence alignment test failed: request_analysis_window returned false.\n";
        return 1;
    }
    ...
    if (result.coreml_beat_events.size() < kEdgeWindowBeats) {
        std::cerr << "Intelligence alignment test failed: too few beat events: "
    ...
    if (!first_bar_is_complete_four_four(result)) {
        std::cerr << "Intelligence alignment test failed: opening bar is not complete 4/4 "
                     "(expected first downbeats at beat indices 0 and 4).\n";
        return 1;
    }
    if (!bars_repeat_every_four_beats(result)) {
        std::cerr << "Intelligence alignment test failed: bar markers are not consistently every 4 beats.\n";
        return 1;
    }
    if (!(result.estimated_bpm > 0.0)) {
        std::cerr << "Intelligence alignment test failed: non-positive BPM.\n";
        return 1;
    }
    if (std::fabs(result.estimated_bpm - kTargetBpm) > kMaxBpmError) {
        std::cerr << "Intelligence alignment test failed: estimated BPM "
    ...
    if (offsets_ms.size() < kEdgeWindowBeats) {
        std::cerr << "Intelligence alignment test failed: too few offsets.\n";
        return 1;
    }
    ...
    std::cout << "Intelligence drift probe: beats=" << result.coreml_beat_events.size()
    ...
    if (const char* dump = std::getenv("BEATIT_INTELLIGENCE_DUMP_EVENTS")) {
        if (dump[0] != '\0' && dump[0] != '0') {
            std::cout << "Intelligence event probe: first_frames="
                      ...
                      << "\n";
            std::cout << "Intelligence downbeat probe: first="
    ...
    std::cout << "Intelligence alignment metrics: bpm=" << result.estimated_bpm
    ...
    if (start_median_abs_ms > kMaxIntroMedianAbsOffsetMs) {
        std::cerr << "Intelligence alignment test failed: intro median abs offset "
                  << start_median_abs_ms << "ms > " << kMaxIntroMedianAbsOffsetMs
                  << "ms\n";
        return 1;
    }
    if (std::fabs(slope_ms_per_beat) > kMaxOffsetSlopeMsPerBeat) {
        std::cerr << "Intelligence alignment test failed: slope " << slope_ms_per_beat
                  << "ms/beat > " << kMaxOffsetSlopeMsPerBeat << "\n";
        return 1;
    }
    if (std::fabs(start_end_delta_ms) > kMaxStartEndDeltaMs) {
        std::cerr << "Intelligence alignment test failed: start/end delta " << start_end_delta_ms
                  << "ms > " << kMaxStartEndDeltaMs << "ms\n";
        return 1;
    }
    if (start_end_delta_beats > kMaxStartEndDeltaBeats) {
        std::cerr << "Intelligence alignment test failed: start/end delta "
                  << start_end_delta_beats << " beats > "
                  << kMaxStartEndDeltaBeats << " beats\n";
        return 1;
    }
    if (odd_even_gap_ms > kMaxOddEvenMedianGapMs) {
        std::cerr << "Intelligence alignment test failed: odd/even median gap "
                  << odd_even_gap_ms << "ms > " << kMaxOddEvenMedianGapMs << "ms\n";
        return 1;
    }
    if (!(early_bpm > 0.0) || !(late_bpm > 0.0)) {
        std::cerr << "Intelligence alignment test failed: invalid edge BPM estimate.\n";
        return 1;
    }
    if (edge_bpm_delta > kMaxTempoEdgeBpmDelta) {
        std::cerr << "Intelligence alignment test failed: edge BPM delta " << edge_bpm_delta
