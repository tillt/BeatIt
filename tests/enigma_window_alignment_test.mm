//
//  enigma_window_alignment_test.mm
//  BeatIt
//
...
constexpr double kMaxOffsetSlopeMsPerBeat = 0.003;
constexpr double kMaxStartEndDeltaMs = 3.8;
constexpr double kMaxStartEndDeltaBeats = 0.008;
constexpr double kMaxOddEvenMedianGapMs = 8.5;
constexpr double kMaxIntroMedianAbsOffsetMs = 6.5;
...
int main() {
    ...
    if (!first_downbeat_feature_frame(result, &first_downbeat_feature_ff)) {
        std::cerr << "Enigma alignment test failed: missing first downbeat feature frame.\n";
        return 1;
    }
    ...
    if (downbeat_feature_delta > kFirstDownbeatFeatureFrameTolerance) {
        const auto lower_feature_frame =
            (kExpectedFirstDownbeatFeatureFrame > kFirstDownbeatFeatureFrameTolerance)
                ? (kExpectedFirstDownbeatFeatureFrame - kFirstDownbeatFeatureFrameTolerance)
                : 0ULL;
        std::cerr << "Enigma alignment test failed: first downbeat feature frame "
    ...
    if (!first_downbeat_sample_frame(result, &first_downbeat_sample_sf)) {
        std::cerr << "Enigma alignment test failed: missing first downbeat sample frame.\n";
        return 1;
    }
    ...
    if (downbeat_sample_delta > downbeat_sample_tolerance_frames) {
        const auto lower_sample_frame =
            (kExpectedFirstDownbeatSampleFrame > downbeat_sample_tolerance_frames)
                ? (kExpectedFirstDownbeatSampleFrame - downbeat_sample_tolerance_frames)
                : 0ULL;
        std::cerr << "Enigma alignment test failed: first downbeat sample frame "
    ...
    if (const char* dump = std::getenv("BEATIT_ENIGMA_DUMP_EVENTS")) {
        if (dump[0] != '\0' && dump[0] != '0') {
            std::cout << "Enigma event probe: first_frames="
                      ...
                      << "\n";
            std::cout << "Enigma downbeat probe: first="
