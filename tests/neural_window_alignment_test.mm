...
constexpr double kMinExpectedBpm = 70.0;
constexpr double kMaxExpectedBpm = 180.0;
...
constexpr double kMaxUnwrappedStartEndDeltaBeats = 0.08;
...
template <typename T>
std::string format_slice(const std::vector<T>& values, std::size_t count, bool from_end) {
    if (values.empty()) {
        return "()";
    }
...
std::string format_double_slice(const std::vector<double>& values,
                                std::size_t count,
                                bool from_end,
                                int precision = 3) {
    if (values.empty()) {
...
int main() {
...
#endif
    const std::filesystem::path audio_path = test_root / "training" / "neural.wav";
    ...
    if (!decode_audio_mono(audio_path.string(), &mono, &sample_rate, &decode_error)) {
        std::cerr << "Neural alignment test failed: decode error: " << decode_error << "\n";
        return 1;
    }
    ...
    if (!stream.request_analysis_window(&start_s, &duration_s)) {
        std::cerr << "Neural alignment test failed: request_analysis_window returned false.\n";
        return 1;
    }
    ...
    if (result.coreml_beat_events.size() < kEdgeWindowBeats) {
        std::cerr << "Neural alignment test failed: too few beat events: "
                  << result.coreml_beat_events.size() << "\n";
        return 1;
    }
    if (result.coreml_beat_events.size() < kMinBeatCount) {
        std::cerr << "Neural alignment test failed: beat event count "
    ...
    if (!bars_repeat_every_four_beats(result)) {
        std::cerr << "Neural alignment test failed: bar markers are not consistently every 4 beats.\n";
        return 1;
    }
    if (!(result.estimated_bpm > 0.0)) {
        std::cerr << "Neural alignment test failed: non-positive BPM.\n";
        return 1;
    }
    if (result.estimated_bpm < kMinExpectedBpm || result.estimated_bpm > kMaxExpectedBpm) {
        std::cerr << "Neural alignment test failed: estimated BPM "
                  << result.estimated_bpm << " outside [" << kMinExpectedBpm
                  << "," << kMaxExpectedBpm << "].\n";
        return 1;
    }
    ...
    if (result.coreml_downbeat_feature_frames.empty()) {
        std::cerr << "Neural alignment test failed: missing downbeat feature frames.\n";
        return 1;
    }
    if (result.coreml_downbeat_feature_frames.size() < kMinDownbeatCount) {
        std::cerr << "Neural alignment test failed: downbeat count "
    ...
    if (!result.coreml_beat_projected_sample_frames.empty() &&
        ...
        std::cerr << "Neural alignment test failed: projected beat count "
    ...
    if (offsets_ms.size() < kEdgeWindowBeats) {
        std::cerr << "Neural alignment test failed: too few offsets.\n";
        return 1;
    }
    ...
    if (const char* dump = std::getenv("BEATIT_NEURAL_DUMP_EVENTS")) {
        if (dump[0] != '\0' && dump[0] != '0') {
            std::cout << "Neural event probe: first_frames="
                      ...
                      << "\n";
            std::cout << "Neural downbeat probe: first="
    ...
    std::cout << "Neural alignment metrics: bpm=" << result.estimated_bpm
    ...
    if (odd_even_gap_ms > kMaxOddEvenMedianGapMs) {
        std::cerr << "Neural alignment test failed: odd/even median gap "
                  << odd_even_gap_ms << "ms > " << kMaxOddEvenMedianGapMs << "ms\n";
        return 1;
    }
    if (!(early_bpm > 0.0) || !(late_bpm > 0.0)) {
        std::cerr << "Neural alignment test failed: invalid edge BPM estimate.\n";
        return 1;
    }
    if (edge_bpm_delta > kMaxTempoEdgeBpmDelta) {
        std::cerr << "Neural alignment test failed: edge BPM delta " << edge_bpm_delta
