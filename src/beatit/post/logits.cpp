...
void clear_projected_grid(CoreMLResult& result) {
...
void set_downbeat_feature_frames(CoreMLResult& result, const std::vector<std::size_t>& frames) {
    result.downbeat_feature_frames.clear();
    result.downbeat_feature_frames.reserve(frames.size());
    for (std::size_t frame : frames) {
        result.downbeat_feature_frames.push_back(static_cast<unsigned long long>(frame));
    }
}

void fill_beats_from_logit_frames(CoreMLResult& result,
...
std::vector<std::size_t> compute_peaks_for_bpm_range(const std::vector<float>& activation,
                                                     double fps,
                                                     float local_min_bpm,
                                                     float local_max_bpm,
                                                     float threshold) {
...
std::vector<float> compute_onset_activation(const std::vector<float>& source) {
    std::vector<float> onset_activation;
    onset_activation.reserve(source.size());
    if (source.empty()) {
        return onset_activation;
    }
    onset_activation.push_back(0.0f);
    for (std::size_t i = 1; i < source.size(); ++i) {
        onset_activation.push_back(std::max(0.0f, source[i] - source[i - 1]));
    }
    return onset_activation;
}

double refine_peak_position(const std::vector<float>& activation, std::size_t frame) {
    if (activation.empty()) {
        return static_cast<double>(frame);
    }
    const std::size_t peak_frame = detail::refine_frame_to_peak(frame, activation, kRefineWindow);
    return interpolate_peak_position(activation, peak_frame);
}

std::vector<std::size_t> build_grid_frames(double phase_seed,
                                           double step_frames,
    ...
    if (step_frames > 0.0 && grid_total_frames > 0) {
        ...
        cursor = phase_seed + k * step_frames;
        while (cursor < 0.0) {
            cursor += step_frames;
        }
...
void log_profile_timing(const BeatitConfig& config, double dbn_ms, double peaks_ms) {
    if (config.profile) {
...
void run_logit_consensus_postprocess(CoreMLResult& result,
    ...
    const std::vector<float> onset_activation = compute_onset_activation(onset_source);

    float max_activation = 0.0f;
    for (float value : result.beat_activation) {
    ...
    std::vector<std::size_t> beat_peaks =
        compute_peaks_for_bpm_range(result.beat_activation, fps, min_bpm, max_bpm, peak_threshold);
    if (beat_peaks.size() < config.logit_min_peaks) {
        ...
        beat_peaks =
            compute_peaks_for_bpm_range(result.beat_activation, fps, min_bpm, max_bpm, lowered);
    }
    ...
    if (bpm <= 0.0) {
        if (interval_frames <= 0.0) {
            ...
            fill_beats_from_logit_frames(result,
                                         fallback_peaks,
                                         config,
                                         sample_rate,
                                         hop_scale,
            ...
            set_downbeat_feature_frames(result, aligned_downbeats);
            clear_projected_grid(result);
            log_profile_timing(config, dbn_ms, peaks_ms);
            return;
        }
    ...
    if (best_score < 0.0) {
        global_phase = refine_peak_position(result.beat_activation, strongest_peak);
    }
    ...
    std::vector<std::size_t> grid_frames =
        build_grid_frames(global_phase, step_frames, used_frames, grid_total_frames);
